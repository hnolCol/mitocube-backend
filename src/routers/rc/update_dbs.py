...
def update_features(background_task : BackgroundTasks, user : UserModel = Depends(is_user_admin)):
...
## shouldnt this be router.update ? or router.put? 
@router.get("/update/annotations",  # , response_model=PlayModel)
            summary="Triggers an update for the annotation database.")
def update_annotations(background_task : BackgroundTasks, user : UserModel = Depends(is_user_admin)):
...
def update_attributes(background_task : BackgroundTasks, user : UserModel = Depends(is_user_admin)):
