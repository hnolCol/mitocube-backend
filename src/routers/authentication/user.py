...
from config.models.user import User, Collaborators, UsersAdminResponse, UserModelForRegistration, UserLabel, UserModelForUpdate, UseRoleReponse
...
@router.get("/users/roles", summary="Returns the available user roles and names", response_model=UseRoleReponse)
def get_user_roles(user : User = Depends(get_user_from_token)):
