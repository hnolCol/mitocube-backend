...
DB = Database.DB()
router = APIRouter(prefix="/api/phenotypes", 
                   tags=["Phenotypes"])
...
@router.post("")
def insert_phenotype(phenotype: PhenotypeInputModel, user: UserModel = Depends(get_user_from_token)) -> bool:
    return DB.phenotypes.insert(phenotype)