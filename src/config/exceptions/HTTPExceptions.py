...
verification_code_incorrect = HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Verification failed. verification_code incorrect. In total you have 5 attempts to verify the token. After that the user will be locked out for 15 min.",
            headers={"WWW-Authenticate": "Bearer"})
...
mfa_locked_out = HTTPException(
        status_code=status.HTTP_401_UNAUTHORIZED,
        detail="The user is locked out due to too many failed MFA attempts.",
        headers={"WWW-Authenticate": "Bearer"})



mfa_already_enabled = HTTPException(
    status_code=status.HTTP_400_BAD_REQUEST,
    detail="MFA is already enabled for this account."
)

mfa_not_setup = HTTPException(
    status_code=status.HTTP_400_BAD_REQUEST,
    detail="MFA setup has not been initiated. Call /mfa/setup first."
)

mfa_code_invalid = HTTPException(
    status_code=status.HTTP_400_BAD_REQUEST,
    detail="Invalid MFA code."
)

mfa_not_enabled = HTTPException(
    status_code=status.HTTP_400_BAD_REQUEST,
